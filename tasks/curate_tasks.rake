  ...
  unless File.exists?(DUMMY_APP + '/Rakefile')
    ...
    Bundler.with_clean_env do
      within_test_app do
        ...
        initializer = File.read("config/initializers/sufia.rb")
        initializer.sub!(/^.*config\.id_namespace *\= *['"]([^'"]*)['"]/, '  config.id_namespace = "\1' << ENV['TRAVIS_RUBY_VERSION'].to_s.gsub(/[^w]*/,'-') << '"').sub(/-+"$/, '"')
        File.write("config/initializers/sufia.rb", initializer)
      end
    end
  end
