...
describe CurationConcern::ArticleActor do
  ...
  describe '#create' do
    ...
    let(:curation_concern) { Article.new(pid: CurationConcern.mint_a_pid )}
    ...
    describe 'valid attributes' do
      ...
      describe 'with a file' do
        let(:attributes) {
          FactoryGirl.attributes_for(:article, visibility: visibility).tap {|a|
            a[:files] = file
          }
        }
        before(:each) do
      ...
      describe 'with a linked resource' do
        let(:attributes) {
          FactoryGirl.attributes_for(:article, visibility: visibility, linked_resource_url: 'http://www.youtube.com/watch?v=oHg5SJYRHA0')
        }
        before(:each) do
          subject.create!
        end
